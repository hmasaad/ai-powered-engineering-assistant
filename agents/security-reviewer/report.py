#!/usr/bin/env python3
"""HTML/JSON report pack for local security reviews."""
...
def _finding_cards(findings: list[dict[str, Any]]) -> str:
    if not findings:
        return '<p class="muted">No security findings.</p>'
    cards: list[str] = []
    for f in findings:
...
<head>
  ...
  <title>Security Review — {_esc(report.get('label'))}</title>
  <style>
    :root {{
      --bg: #f6f4f2;
      --ink: #1c1917;
    ...
    body {{
      ...
      background:
        radial-gradient(circle at 88% 0%, #fecaca55, transparent 40%),
...
<body>
<main>
  <p class="muted">Local Security Reviewer · detect security concerns · read-only</p>
...
<style>
 body {{ font-family: "IBM Plex Sans", sans-serif; margin: 2rem; background: #f6f4f2; }}
...
<p class="muted">Local reports · security concerns only · read-only agent</p>
<table>
